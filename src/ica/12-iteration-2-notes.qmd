...
# Example 3: purrr::map()
results3 <- bench::mark(purrr::map_int(1:1e5, ~.x)) # achieve more complex functions 


# Example 4: Standard R Function/Operator
results4 <- bench::mark(seq(1,1e5)) # cant do complicated mean etx
results5 <- bench::mark(1:1e5) # without the seq function 
...
```{r}
# Compile Results
bind_rows(results1, results2, results3, results4, results5) |>
  mutate(type = c("for_loop_no_storage", "for_loop_with_storage", "purrr_map", "standard_func", "without_seq_func")) |>
