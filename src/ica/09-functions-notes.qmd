...
function_name <- function(input_name_1, input_name_2 = default_value_2){
  ...
  output <- input_name_1 + input_name_2
...
```{r error = TRUE}
# Does throw an error
average(c(1, 2, 3, NA), remove_nas = TRUE) # remove_nas was default false not true
...
average_new(1:3)
fun_new_thing_within_function # object not found -- not accessible 
```
...
```{r}
rescale <-function(x, remove_nas = TRUE){
  x<- (abs(c(x)/100))
  return(x)
  
}

rescale(2:4) # divide by range 
