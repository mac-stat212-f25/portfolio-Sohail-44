...
```{r}
ggplot(airquality_mod, aes(x = Month + Wind + Temp, y = Solar.R_mod2)) + 
    geom_boxplot() 

ggplot(airquality_mod, aes(x = Month + Wind + Temp, y = Ozone)) +
    geom_point() +
    geom_smooth() +
    geom_vline(data = airquality_mod |> filter(is.na(Ozone)), mapping = aes(xintercept = Month + Wind + Temp)) 
